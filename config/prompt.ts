...
function generatePrompt(): string {
  const skillsNames = HeroConfig.skills.map((skill) => skill.name).join(', ');
  const socials = socialLinks
    .map((links) => `${links.name}: ${links.href}`)
    .join(', ');
...
- Focus on Utkarsh's expertise and projects
- For work inquiries, direct to email: [utkarshrajmishra811545@gmail.com](mailto:ramxcodes@gmail.com), LinkedIn: [LinkedIn](https://www.linkedin.com/in/utkarsh-raj-mishra-3b7018283/)
